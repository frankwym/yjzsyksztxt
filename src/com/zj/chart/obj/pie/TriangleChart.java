...
import com.zj.chart.render.pie.TrianglePlot;

public class TriangleChart implements IChart {
	...
	public void drawChart(Graphics2D g, double x, double y, int width,
		...
		//创建饼图
		TrianglePlot plot1 = new TrianglePlot(pieDataset);
		...
		// 获得图表对象的引用  
		TrianglePlot plot = (TrianglePlot) pieChart.getPlot();
		//获取饼图绘制样式
...
//        double stringheight = rec.getHeight();	
		...
		Line2D.Double line2 = new Line2D.Double(X-l/2,Y-l/2,X,Y-l/2);
		Line2D.Double line3 = new Line2D.Double(X,Y-l/2,X-l/2,Y+l/2);
		Line2D.Double line4 = new Line2D.Double(X-l/2,Y-l/2+l/3*i,X,Y-l/2+l/3*i);
		...
		g2d.drawString(temp,  (int) (X),(int) (Y-l/2+l/3*i));
		g2d.draw(line1);
		g2d.draw(line2);
		g2d.draw(line3);
		g2d.draw(line4);
//		g2d.setFont(new Font("宋体",Font.PLAIN,10));
//		g2d.drawString(Double.toString(sum1),  (int) (x),(int) (y-l/2+l/3*i));
...
//			g2d.drawString("单位:"+ "（" + fieldUnits[0] + "）",(int)(rect1.getCenterX()-rect1.getWidth()/4),(int)(rect1.getCenterY()+rect1.getHeight()/2));
	...
	public JFreeChart setStruLegend(PieDataset pieDataset,String title,HashMap<String, Color> hashMap,ChartDataPara thematicRendition)  {
		TrianglePlot plot = new TrianglePlot(pieDataset);
	...
	public HashMap<String, Shape> generateHotArea(double x, double y,
      ...
      //饼图大小由整个饼图所占绘图区域的比例来决定
		...
		for(int i=0;i<thematicRendition.getFieldName().length;i++){				
			...
			ANGLE = indicatorDatas[0].getValues()[i]/sum*360;	
			if(i==0){
				int[] xPoints = {(int) x,(int) (x),(int) (x-2*R*Math.tan(30*Math.PI/180))};
				int[] yPoints = {(int) (y+R),(int) (y-R),(int) (y-R)};
