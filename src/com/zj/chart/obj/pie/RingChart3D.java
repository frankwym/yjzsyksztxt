...
public class RingChart3D implements IChart {
...
//		double LabelGap = chartStyle.getLabelGap();
		...
		double SectionDepth = chartStyle.getSectionDepth();
		//设置饼图颜色
...
//			}
      	 ...
      	 plot.setSectionDepth(SectionDepth*2);
...
//    		double R1 = width/2;
    		...
    		Line2D.Double line = new Line2D.Double(X-R-stringlength/8, Y-R+R*i/2, X,  Y-R+R*i/2);
    		...
    		g2d.drawString(temp,  (int) (X-R-stringlength/2),(int) (Y-R+R*i/2));
...
//      title.setFont(new Font("黑体",Font.PLAIN,15));
        ...
        TRingPlot plot1 = (TRingPlot) chart.getPlot();
...
//				Rectangle2D.Double(x-width/4, y-width/2/2.75, width/2, width/2.75); 
				...
				Rectangle2D.Double symbolEnvlope1 =  new 
				Rectangle2D.Double( x-width*(1-SectionDepth)/2*(1-interiorGap*2), y-width*TiltAngle*(1-SectionDepth)/2*(1-interiorGap*2)*(1-DepthFactor/4), width*(1-interiorGap*2)*(1-SectionDepth), width*TiltAngle*(1-interiorGap*2)*(1-DepthFactor)*(1-SectionDepth)); 
...
//				 Rectangle2D.Double rectangleDouble = new Rectangle2D.Double();
				...
				for(int i=0;i<thematicRendition.getFieldName().length;i++){				
					...
					Arc2D.Double arc2 = new Arc2D.Double();
					 ...
					 arc2 = new Arc2D.Double(symbolEnvlope1, 0, 360, 2);
...
//						area1.subtract(area2);
		    			...
		    			hotAreahHashMap.put(text, areaArc1);
		}
		return hotAreahHashMap;
